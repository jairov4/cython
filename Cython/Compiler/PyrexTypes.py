...
class CValueClassType(CStructOrUnionType):
    ...
    is_value_class = 1
    supports_refnanny = False  # like memoryview: no single pointer to track
    ...
    def __init__(self, name, scope, cname, ext_type, py_name=None):
        ...
        # Refcount helper function names (emitted in ModuleNode alongside converters).
    ...
    def needs_refcounting(self):
        # True iff any field is a pyobject or itself needs refcounting (e.g. a
        # nested value_type with object fields or a memoryview slice).
        # A pure-C value struct has no refcounted fields → returns False, keeping
        # the v1/v2 fast path byte-for-byte unchanged.
        return any(
            f.type.is_pyobject or getattr(f.type, 'needs_refcounting', False)
            for f in self.scope.var_entries
        )
    ...
    # When needs_refcounting is False (pure-C fields), all operations are no-ops
    # identical to the base CStructOrUnionType behaviour.
    # When needs_refcounting is True (object/memoryview fields present), we emit
    # calls to per-type helper functions generated by ModuleNode.
    ...
    def get_decref_set_code(self, cname, rhs_cname):
        # Mirror __Pyx_DECREF_SET for Python objects:
        # The caller (NameNode.generate_assignment_code) always calls
        # rhs.make_owned_reference() first, which INCREFs rhs's fields.
        # So we only need to DECREF the old value and copy the new one.
        if not self.needs_refcounting:
            return "%s = %s;" % (cname, rhs_cname)
        return ("%s(&%s); %s = %s;"
                % (self._refcount_decref_fname, cname,
                   cname, rhs_cname))

    def get_xdecref_set_code(self, cname, rhs_cname):
        return self.get_decref_set_code(cname, rhs_cname)

    # Refnanny track/giveref/gotref are no-ops: a struct has no single pointer.
    get_gotref_code = get_xgotref_code = get_giveref_code = get_xgiveref_code = (
        lambda *args, **kwds: None)

    def nullcheck_string(self, cname):
        # A value struct is always "present" (no null struct concept).
        return "1"
