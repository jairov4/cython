...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def prepare_utility_code(self):
        ...
        for name, entry in sorted(env.entries.items()):
            if getattr(entry, 'exported_to_pydict', False) and not entry.type.is_pyobject:
    ...
    def generate_descr_get_function(self, scope, code):
        ...
        # different return types. Skip generating it if any properties are overridable.
        has_overridable = any(
            getattr(prop_entry.scope, 'is_overridable', False)
            for prop_entry in scope.property_entries
            if getattr(prop_entry, 'scope', None)
        )
        if has_overridable and scope.property_entries:
            return
    ...
    def generate_typeobj_spec(self, entry, code):
        ...
        has_overridable_props = scope.property_entries and any(
            getattr(prop_entry.scope, 'is_overridable', False)
            for prop_entry in scope.property_entries
            if getattr(prop_entry, 'scope', None)
        )
        for slot in TypeSlots.get_slot_table(code.globalstate.directives):
            ...
            if has_overridable_props and slot.slot_name in ('tp_descr_get', 'tp_descr_set'):
                continue
    ...
    def generate_init_subclass_function(self, scope, code, permissive=False):
        ...
        if permissive:
            ...
            code.putln("#endif")
            code.putln("    {")
            code.putln("#ifdef PYPY_VERSION")
            code.putln("      /* PyPy does not export PySuper_Type; look up 'super' from builtins. */")
            code.putln("      PyObject *__pyx_super_type_obj = PyDict_GetItemString(")
            code.putln("          PyEval_GetBuiltins(), \"super\");")
            code.putln("      if (!__pyx_super_type_obj) return NULL;")
            code.putln("      PyObject *__pyx_super = PyObject_CallFunctionObjArgs(")
            code.putln("          __pyx_super_type_obj, __pyx_super_from, cls, NULL);")
            code.putln("#else")
            code.putln("      PyObject *__pyx_super = PyObject_CallFunctionObjArgs(")
            ...
            code.putln("#elif defined(PYPY_VERSION)")
            code.putln("  /* PyPy doesn't reliably expose tp_dictoffset; detect pure Python")
            code.putln('     classes by checking whether "__dict__" is in the class namespace.')
            code.putln("     type.__new__ adds a __dict__ descriptor to pure Python subclasses;")
            code.putln("     Cython types created via PyType_FromSpec do not have one. */")
            code.putln("  int __pyx_is_python_class;")
            code.putln("  {")
            code.putln("    PyObject *__pyx_cls_ns = PyObject_GetAttrString(cls, \"__dict__\");")
            code.putln("    __pyx_is_python_class = __pyx_cls_ns ?")
            code.putln("        PyMapping_HasKeyString(__pyx_cls_ns, \"__dict__\") : 0;")
            code.putln("    Py_XDECREF(__pyx_cls_ns);")
            code.putln("  }")
            code.putln("#else")
