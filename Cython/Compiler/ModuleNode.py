...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def generate_init_subclass_function(self, scope, code, permissive=False):
        ...
        if permissive:
            ...
            code.putln("  /* On PyPy, Cython types created via PyType_FromSpec never invoke")
            code.putln("     __init_subclass__, so any invocation here is always from a Python")
            code.putln("     'class' statement; cls is always a pure Python class. */")
            code.putln("  int __pyx_is_python_class = 1;")
            code.putln("#else")
